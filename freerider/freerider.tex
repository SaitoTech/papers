...
When faced with the question of why only the highest-possible-bid represent truthful preference revelation, Ethereum-focused researchers like Roughgarden and Shi simply point to mechanisms in which the assumption is reasonable, such as the second-price Vickrey-Clarke-Groves (VCG) auction run by a trusted auctioneer. By comparing on-chain and off-chain implementations of similar auction mechanisms, these papers assume that if a bid constitutes adequate preference revelation in a mechanism with a trusted auctioneer then it should also be truthful in the presence of a potentially adversarial counterparty. The user is still reporting their utility of getting the good, no?
