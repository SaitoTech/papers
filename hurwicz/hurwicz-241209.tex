...
\begin{abstract}
In recent years, a number of impossibility proofs have been written in computer science which claim the impossibility of achieving desirable economic properties like incentive compatibility and collusion-resistance in informationally-decentralized and self-sustaining blockchain networks. In the face of growing consensus that these problems are impossible to solve, this paper offers a dissenting proof, demonstrating the existence of a mechanism that implements the social choice rule of pareto optimality, thereby achieving both incentive compatibility and collusion-resistance.
\end{abstract}
...
Is this pessimism warranted? The rest of this paper demonstrates it is not, introducing \ourTFM as a TFM that achieves incentive compatibility and \textit{pareto optimality}. In order to avoid the contributing to the confusion that is being caused by the casual use of terms like "incentive compatibility" in many papers, the section that follows provides a brief review of what incentive compatibility means in both economics and auction design as a prerequisite to demonstrating that both forms are achievable in a single TFM.
...
The shift towards hybrid provision of public and private goods creates new analytic problems for TFM designers, since byzantine strategies emerge that allow participants to free-ride on the contributions their peers to the public good component. Collusion between users and producers via \textit{side-contract payments} is one example of a new form of strategic vulnerability that emerges as a result.
...
There are three key differences between \ourTFM and  existing TFMs. \textbf{First,} we introduce the concept of "routing-work" which replaces \textit{mining} or \textit{staking} as the form of work used to regulate the pace of block production. The amount of routing work in any block is verifiable in the absence of a trusted-third party and thus provides an objective standard for determining when a block can be produced in a permissionless environment. \textbf{Second,} we propose a novel payment rule that offers payments to multiple network nodes. \textbf{Third}, we demonstrate how competition between block producers in this new mechanism eliminates opportunities for the socially-suboptimal forms of value-extraction that characterize collective action problems and create problems achieving \textit{pareto optimality} in other mechanisms. Figure~\ref{fig::rtr-tfm} summarizes the main components of \ourTFM. 
